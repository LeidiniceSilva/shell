...
cpdf(){
  ...
  CDO -b F32 timsum -gec,-1000 $fin ${fout}_cnt.nc
  vv=pr
  CDO -b F32 div -histcount,$( echo $( seq $mn $i $mx ) | sed 's/ /,/g' ) $fin ${fout}_cnt.nc  $fout
  rm ${fout}_cnt.nc
}
...
of1=$( basename $f1 .nc )_sp-pdf.nc
of2=$( basename $f2 .nc )_sp-pdf.nc

...
export mdir=/marconi_work/ICT23_ESP/ggiulian/OBS/SREX
obsn=ERA5
...
#--
export dname="${subregs%% *}"

#-- read txt file that contains box info
bdir=$hdir/vert
export box_f=$bdir/box_latlon.txt
subregs=$( head $box_f -n1 )
subregs="${subregs#*,}"
subregs="${subregs//,/ }"
echo "subregs = $subregs"

#-- create mask for boxes
ncl -Q $scrdir/create_box_mask.ncl
...
#      fi
	  ...
	  #
	  export f1=$mskfc
	  #export f2=$( eval ls $hdir/*.nc | head -n1 )
	  export f2=$( eval ls $hdir/*.nc | grep -v "clm" | head -n1 )
	  ncl -Q $scrdir/apply_RegCM_land_sea_mask_2.ncl
	  #

...
# errtime=`echo $( tail -20 ${colog} | grep 'variables written at' | tail -1 ) | cut -d ' ' -f 5`
  ...
  if [ ! -z "$notoerrchk" -o ! -z "$qlhserrchk" ]; then
    dtdrop=15
    gcm=$( echo $cjn | cut -d_ -f1 | cut -d- -f1 )
    [[ $gcm = NorESM ]] && dtdrop=30
...
lastsave=$( ls $svdir/*SAV.*.nc | tail -1 ) #$( ls $svdir/*SAV.$( echo $restartdate | cut -c1-6 )*.nc | tail -1 )

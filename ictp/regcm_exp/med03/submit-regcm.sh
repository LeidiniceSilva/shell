...
base=/marconi_work/ICT23_ESP/jciarlo0/WMD03
...
nl=$1 #namelist
startDate="${2:-1995010100}" #start Date
dep=$3
...
thisdt=60
#if [ $startDate -lt 2001022600 ]; then
#  [[ $nl = ERA5-WMediterranean.in ]] && thisdt=15
#fi
[[ -f .${dpath}/.newdt ]] && thisdt=$( cat .${dpath}/.newdt )
[[ -f .${dpath}/.newdt ]] && rm .${dpath}/.newdt

startTarget=1995010100
  endTarget=2016010100
#[[ $nl = ERA5-WMediterranean.in ]] && endTarget=2001030100

## check icbc
icbcdir=$( cat $nl | grep dirter | cut -d"'" -f2 )
icbcfil=$( ls $icbcdir/*ICBC*.nc | tail -1 )
icbcdat=$( basename $icbcfil | cut -d. -f2 )
if [ $icbcdat -lt $endTarget ]; then
  endTarget=$icbcdat
fi

# use climaaer 2 for historical
if [ $endTarget -gt 2015010100 -a $startDate -lt 2015010100 ]; then
  endTarget=2015010100
fi
#if [ $startDate -lt 2014123124 ]; then
#  clima=2
#else
#  clima=3
#fi
...
sed -i "s/thisdt/${thisdt}/g" $nnl
#sed -i "s/clim_set/${clima}/g" $nnl
